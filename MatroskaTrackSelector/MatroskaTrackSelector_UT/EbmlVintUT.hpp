...
namespace EbmlVintUT
{
    ...
    void run_tests()
    {
        ...
        {{  // Lowest                Highest                       Random . . .
            {(uint64_t)1 << (7 * 0), ((uint64_t)1 << (7 * 1)) - 1, 2, 6, 12, 38, 39, 63, 66, 75, 79, 85, 94, 106, 109, 110, 113, 118, 119, 126},
        ...
        for (uint32_t i = 0; i < test_numers.size(); ++i)
        {
            for (uint64_t test_number : test_numers[i])
            {
                uint64_t value_with_marker = test_number | ((uint64_t)1 << (7 * (i + 1) + 1));

                EbmlVint<false> without_marker = test_number;
                EbmlVint<true> with_marker = value_with_marker;

                REQUIRE(without_marker.m_value == test_number);
                REQUIRE(with_marker.m_value == value_with_marker);
                REQUIRE(with_marker._s_remove_vint_marker(with_marker.m_value) == test_number);
            }
        }
    }
