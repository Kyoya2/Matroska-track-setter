...
def parse_range_attribute(range_attr: str, element_type: EbmlElementType) -> EbmlValueConstraintType:
    """
    Return a value constraint type of strings such as "1", "not 0", "<= 1", "< 5, >= 0", "3-90"
    """
...
def get_generic_ebml_elements() -> List[EbmlElement]:
    ...
    for element_section_element in ebml_header_elements + global_ebml_elements:
        ...
        property_names_and_values_elements = element_section_element.findall('dl/*')

        # make the list of [pname1, pval1, pname2, pval2, ...] into [[pname1, pval1], [pname2, pval2], ...]
        property_name_value_element_pairs = [property_names_and_values_elements[i:i+2] for i in range(0, len(property_names_and_values_elements), 2)]
...
def get_elements_from_ebml_schema(schema_file_data: str) -> List[EbmlElement]:
    """
    This function generates a convenient dict that contains all specified elements
    """
...
def get_ebml_elements() -> List[EbmlElement]:
    generic_ebml_elements = get_generic_ebml_elements()
    matroska_elements = get_elements_from_ebml_schema(requests.get(MATROSKA_EBML_SCHEMA_ADDRESS).content.decode())

    all_elements = generic_ebml_elements.copy()
    # Only add generic EBML elements that are not overriden by the Matroska elements
    for generic_element in generic_ebml_elements:
